...
class TargetExtractionBlock(Layer):
    ...
    def call(self, hidden_states, mask=None, training=False, **kwargs):
        ...
        # linear_logits = self.linear_predict(h)
        decoded_sequence, output_logits, seq_length, chain_kernel = self.crf(h, mask=mask)  # crf chain_kernel 未更新
        # decoded_sequence, output_logits, seq_length, chain_kernel = self.crf(h)  #crf chain_kernel 未更新        
...
class TargetAspectClassificationBlock(Layer):
    ...
    def call(self, all_tokens, aspect_tokens, target_pos_info, target_batch_info, attention_mask,
        ...
        :param batch_clf_index: (batch_num_targets)
        :param kwargs:
        :return:
        """
...
class AspectSentimentModel(Model):
    ...
    def call(
        ...
        if phase == "train" or phase == "valid":
            ...
            ner_labels, classify_labels, sentiment_labels, batch_senti_index, batch_clf_index = inputs
        ...
        sb_len = self.sentence_b_idx_map[-1][-1] + 1  # tokenizer(sentence_b) + [sep]
        sb_start = tf.shape(tokens_hidden_state)[1] - sb_len
        crf_mask = tf.concat([input_ids[:, :sb_start] != 0, tf.fill((batch_size, sb_len), False)], axis=1)

        # (batch_size, num_aspects, hidden_size)
        asp_rep = self.gather_aspect_tokens_hidden_state(tokens_hidden_state)
        ...
        if phase == "train" or phase == "valid":
            ...
            ner_predicts = ner_predicts[..., :sb_start]
        ...
        if phase == "train" or phase == "valid":
            ...
            if target_aspect_pair is None:
                ...
                if output_attentions:
                    result["ta_attention"] = ta_attentions
                return result
        ...
        if phase == "train" or phase == "valid":
            ...
            if output_attentions:
                result["ta_attention"] = ta_attentions
                result["sc_attention"] = sc_attentions

            # return output if not output_attentions else (output, (ta_attentions, sc_attentions))
            return result

    def gather_aspect_tokens_hidden_state(self, all_tokens):
        pool_tensors = tf.TensorArray(dtype=tf.float32, size=0, dynamic_size=True)
        sb_len = self.sentence_b_idx_map[-1][-1] + 1  # tokenizer(sentence_b) + [sep]
        sb_start = tf.shape(all_tokens)[1] - sb_len
        for i in tf.range(self.sentence_b_idx_map.shape[0]):
            start, end = self.sentence_b_idx_map[i, 0] + sb_start, self.sentence_b_idx_map[i, 1] + sb_start
            gather_tensors = all_tokens[:, start: end, :]

            # (batch_size, hidden_state)
            pool_tensors = pool_tensors.write(i, self.target_aspect_pooling(gather_tensors, axis=1))

        # (num_aspects, batch_size, hidden_state)
        pool_tensors = pool_tensors.stack()

        # (batch_size, num_aspects, hidden_state)
        pool_tensors = tf.transpose(pool_tensors, perm=[1, 0, 2])
        return pool_tensors
