...
class TFBertMainLayer(tf.keras.layers.Layer):
    ...
    def call(
        ...
        **kwargs,
        ...
        shape = tf.shape(inputs["attention_mask"])
        shape_shape = tf.shape(tf.shape(inputs["attention_mask"]))
        if shape.shape == 2:
            extended_attention_mask = inputs["attention_mask"][:, tf.newaxis, tf.newaxis, :]   # lzl todo
        else:
            extended_attention_mask = inputs["attention_mask"][:, tf.newaxis, :, :]
